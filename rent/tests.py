...
class HomePageTest(TestCase):
    def setUp(self):
        ...
        self.res_1 = Reservation.objects.create(
            rental=self.rental_1, checkin=date(2022, 1, 1), checkout=date(2022, 1, 13)
        )
        self.res_2 = Reservation.objects.create(
            rental=self.rental_1, checkin=date(2022, 1, 20), checkout=date(2022, 2, 10)
        )
        self.res_3 = Reservation.objects.create(
            rental=self.rental_1, checkin=date(2022, 2, 20), checkout=date(2022, 3, 10)
        )
        self.res_4 = Reservation.objects.create(
            rental=self.rental_2, checkin=date(2022, 1, 2), checkout=date(2022, 1, 20)
        )
        self.res_5 = Reservation.objects.create(
            rental=self.rental_2, checkin=date(2022, 1, 20), checkout=date(2022, 2, 11)
        )
